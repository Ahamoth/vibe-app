      ...
      .map(vibe => `
        <div class="card">
          ...
          <div style="margin-top: 10px; display: flex; gap: 15px;">
            <button onclick="toggleLike('${vibe.id}')" style="background: none; border: none; color: #94a3b8;">❤️ 0</button>
            <button onclick="showComments('${vibe.id}')" style="background: none; border: none; color: #94a3b8;">💬 0</button>
          </div>
        </div>
      `).join("");
  } catch (error) {
    content.innerHTML = '<div class="card">Ошибка загрузки. Проверьте подключение.</div>';
